...
async function reloginByIFrame() {
    ...
    let emailField = await currentPage.findById('account_name_text_field');
    console.log('ready to type account name button');
    await emailField.sendKeys(accountInfo.email, Key.ENTER);
  
    await (await currentPage.driver).sleep(5000);
  
    let passwordField = await currentPage.findById('password_text_field');
    await passwordField.sendKeys(accountInfo.pwd, Key.ENTER);
  
    await (await currentPage.driver).sleep(10000);
  
    let continueBtn = await currentPage.findByTagName('button');
