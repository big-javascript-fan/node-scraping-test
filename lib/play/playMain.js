...
async function playArtist(list, setting) {
  ...
  for(var i=0; i<list.length; i++) {
    ...
    try {
      let leftButtons = await currentPage.findByClassName('web-navigation__list-item-button');
      await leftButtons[0].click();
      await sleep(15000);
      // open music URL and click play button
      await currentPage.visit(list[i].link);
      await sleep(15000);
    
      let signOutButton = await currentPage.findByClassName('web-navigation__auth-button--sign-out');
    } catch(error) {
      currentPage = await login(workerData.accountInfo, newProxyUrl);
